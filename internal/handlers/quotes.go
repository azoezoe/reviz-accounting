...
func (s *Server) quoteRevise(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		_, err = tx.Exec(`INSERT INTO quote_items(quote_id,description,detail,quantity,unit,unit_price_cents,is_choice,sort_order) SELECT $1,description,detail,quantity,unit,unit_price_cents,is_choice,sort_order FROM quote_items WHERE quote_id=$2`, newID, id)
	}
	if err == nil {
